...
export const addUser = async (data) => {
  ...
  try {
    ...
    const [result] = await pool.query(
      `INSERT INTO member (email, name, gender, birth, address, detail_address, phone_num) VALUES (?, ?, ?, ?, ?, ?, ?);`,
      [
        data.email,
        data.name,
        data.gender,
...
export const setPreference = async (userId, foodCategoryId) => {
  ...
  try {
    await pool.query(
      `INSERT INTO preferrenceof_food (food_category_id, member_id) VALUES (?, ?);`,
  ...
  try {
    const [preferences] = await pool.query(
      "SELECT ufc.id, ufc.food_category_id, ufc.member_id, fcl.name " +
        "FROM preferrenceof_food ufc JOIN food_category fcl on ufc.food_category_id = fcl.id " +
